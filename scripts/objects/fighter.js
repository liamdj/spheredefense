...
export class Fighter {
    ...
    timeStep = (time) => {
        const nextLocalRotation = this.group.rotation.toVector3().addScaledVector(this.angularVel, 0.01);
        // cannot rotate through poles
        nextLocalRotation.setX(Math.max(-Math.PI / 2, Math.min(Math.PI / 2, nextLocalRotation.x)));
        ...
        const worldVelocity = new THREE.Vector3(0, 0, -this.speed).applyQuaternion(this.group.quaternion).applyQuaternion(this.mesh.quaternion);
        const worldPosition = this.group.getWorldPosition(new THREE.Vector3());
        const nextWorldPosition = new THREE.Vector3().addVectors(worldPosition, worldVelocity);
        ...
        const worldRotationDelta = new THREE.Quaternion().setFromUnitVectors(worldPosition.normalize(), nextWorldPosition.normalize());
    ...
    fireBullets = (intersectPoint) => {
        ...
        if (intersectPoint) { // adjust bullet direction toward target
            leftBullet = new Bullet(leftOffset, new THREE.Vector3(-2, -2, 0).add(intersectPoint));
            rightBullet = new Bullet(rightOffset, new THREE.Vector3(2, -2, 0).add(intersectPoint));
        } else { // fire straight ahead
            const leftOffset2 = this.group.localToWorld(new THREE.Vector3(-4, -4, -24));
            const rightOffset2 = this.group.localToWorld(new THREE.Vector3(4, -4, -24));
